...
>  FreeRTOS 现在虽然还支持协程式调度, 但是官方已经表示不会再更新
...
同等优先级任务轮流地享有相同的 CPU 时间(可设置),  叫**时间片**, 在FreeRTOS中, 一个时间片就等于SysTick 中断周期, 注意没有用完的时间片不会再使用
...
PendSV和SysTick设置最低优先级的作用: 保证系统任务切换不会阻塞系统其他中断的响应
